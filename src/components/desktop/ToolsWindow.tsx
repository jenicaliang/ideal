...
import ToolsPage from '../ideal/ToolsPage'

export default function ToolsWindow({ onClose, onFocus, onMinimize, zIndex, isMinimized }: {
  onClose: () => void
  onFocus: () => void
  onMinimize: () => void
  zIndex: number
  ...
  useEffect(() => {
    if (!isMinimized) setVisibleKey(k => k + 1)
  }, [isMinimized])
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, position: 'relative', overflow: 'hidden', backgroundColor: 'var(--bg, #f5f3ef)' }}>
        <ToolsPage visibleKey={visibleKey} />
      </div>
    </div>
  )
}